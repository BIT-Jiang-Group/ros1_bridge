...
> This repository is the customized ROS1 bridge used by the DCL-SLAM and
> M-TARE deployment. It is based on the ROS 2 Foxy-era `ros1_bridge` 0.10.1
> source tree and is not interchangeable with the unmodified binary package.

## Project-specific changes

- Load an explicit topic/service allowlist from the ROS1 parameter server.
- Configure queue depth per topic.
- Support ROS2 `transient_local` durability and the corresponding ROS1 latched
  publisher for startup descriptors.
- Suppress messages published by the bridge itself to prevent a bidirectional
  ROS1-DDS-ROS1 feedback loop.
- Generate conversion factories for the custom DCL-SLAM and M-TARE messages
  visible when this package is compiled.

The initial imported snapshot did not retain its original `.git` metadata. The
package manifest reports version `0.10.1`; the official upstream is
<https://github.com/ros2/ros1_bridge>. Keep `origin` pointed at this customized
repository and use an `upstream` remote only to inspect future upstream changes.
Do not merge a newer ROS distribution branch without rebuilding and repeating
the two-robot communication tests.
...
- ROS1 Noetic
- ROS2 Foxy
- DCL-SLAM ROS1 messages and `dcl_slam_msgs` ROS2 mirror sourced at build time

Custom message conversion code is generated at compile time. Whenever a `.msg`
definition or mapping rule changes, rebuild this package with both the ROS1 and
ROS2 message workspaces sourced and pass `--cmake-force-configure` to `colcon`.
The DCL-SLAM repository contains the complete build order and verification
commands.
