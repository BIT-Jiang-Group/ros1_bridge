  ...
  bool latch_ros1)
{
  ...
  auto ros1_pub = factory->create_ros1_publisher(
    ros1_node, ros1_topic_name, publisher_queue_size, latch_ros1);
...
{
  RCLCPP_INFO(
    ros2_node->get_logger(), "create bidirectional bridge for topic %s with custom QoS",
    topic_name.c_str());
  BridgeHandles handles;
  handles.bridge1to2 = create_bridge_from_1_to_2(
    ros1_node, ros2_node,
    ros1_type_name, topic_name, queue_size, ros2_type_name, topic_name, qos);
  handles.bridge2to1 = create_bridge_from_2_to_1(
    ros2_node, ros1_node,
    ros2_type_name, topic_name, qos, ros1_type_name, topic_name, queue_size,
    handles.bridge1to2.ros2_publisher, latch_ros1);
  return handles;
}
