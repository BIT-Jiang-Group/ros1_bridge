...
int main(int argc, char * argv[])
{
  ...
  // transient_local: retain the last ROS 2 sample and latch the ROS 1 output
    ...
    topics.getType() == XmlRpc::XmlRpcValue::TypeArray)
  {
    for (size_t i = 0; i < static_cast<size_t>(topics.size()); ++i) {
      ...
      if (topics[i].hasMember("transient_local")) {
        transient_local = static_cast<bool>(topics[i]["transient_local"]);
      }
      printf(
      ...
      try {
        ...
        if (transient_local) {
          ...
          handles = ros1_bridge::create_bidirectional_bridge(
            ros1_node, ros2_node, "", type_name, topic_name, queue_size, qos, true);
        } else {
          handles = ros1_bridge::create_bidirectional_bridge(
